...
- 预览卡片支持内部控件、滚动、背景点击打开完整纸片和独立关闭区；背景点击只打开或置前完整纸片，保留当前浏览卡片直到鼠标真正离开。旧卡开始收回后会立即停止接收点击；拖拽任意队列胶囊前都会同步收回当前浏览卡片并恢复正常占位，从高预览卡开始拖动时也不会再让抓取位置突然偏移，现有跨屏、换边和回边拖拽继续只处理标准胶囊。
