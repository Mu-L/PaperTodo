...
- 视觉页可按 5% 调整 80%–120% 的整体字体大小，并分别设置笔记正文、待办正文、标题和胶囊文字的小中大字号与加粗。单个笔记原有的百分比缩放会在全局与正文字号基础上继续叠加；纸片顶栏高度在整体放大时仅按约一半幅度加高。文字渲染提供「标准 / 柔和 / 锐利」三档：标准沿用默认路径，柔和使用布局稳定的灰阶平滑，锐利使用像素对齐更强的灰阶显示。从窗口切换中隐藏纸片默认开启；显示主胶囊选项在胶囊分组中上移一位。
...
- 胶囊模式下，按下 Esc可快速收回胶囊。
- 快捷键页支持为显示全部、隐藏全部、切换显隐、新建待办、新建笔记、退出录入全局快捷键。
- 快捷键新建待办 / 笔记会把纸片放到当前鼠标位置（居中并夹在工作区内），不再固定到屏幕左上角。
- 新增默认关闭的「快速启动侧边胶囊」：
...
- 纸片标题编辑与存储统一上限为 20 个字符（全角字计为 1）；输入框不再允许打到更长再在提交时截断。设置里的标题最大长度仍在 2–20 之间可调，提交时按当前设置截断。
