...
- 设置窗口拆分为「基础行为 / 视觉 / 快捷键」三页，高度按三页真实内容测量后取最高一页，切换分页时不再忽高忽低；常规显示器上完整铺开不出现滚动条，仅当超出设置窗口所在显示器的工作区时才滚动，并在同页刷新后保留滚动位置。右下角署名改为贴边显示，不再占用整行，滚动时会避开滚动条；调整整体字号时保持窗口顶部不动，只向下扩展。Markdown 显示仍在基础行为页，分区顺序按日常使用与胶囊优先整理。视觉页可按 5% 调整 80%–120% 的整体字体大小，并分别设置笔记正文、待办正文、标题和胶囊文字的小中大字号与加粗；各组共用一个说明图标并有轻分隔。单个笔记原有的百分比缩放会在全局与正文字号基础上继续叠加，待办行的内边距、最小高度、勾选列和勾选框也会随整体字体大小同步缩放；设置中的 Markdown 扩展名输入框会为放大的字体补足高度；纸片顶栏高度在整体放大时仅按约一半幅度加高，顶栏按钮宽高保持固定；文字边缘风格提供「系统平滑 / 统一灰阶」两档，默认让 Windows 和字体自行选择，并让边缘胶囊恢复新渲染合并前的显示路径；统一灰阶会覆盖透明纸片、设置、胶囊和笔记正文，并为笔记以及使用等线、MiSans 等字体的边缘胶囊增加更柔和的双向平滑。
