...
- 可在实验室启用轻量待办提醒：支持从待办行按钮或右键菜单选择默认间隔、10/30/60 分钟、当天傍晚、次日早晨和自定义时间；设置后在待办行显示可点击的简洁倒计时，可随时修改或取消，到期时通过系统托盘单次提示。可单独隐藏铃铛按钮、仅保留右键入口；关闭提醒功能时停止计时并隐藏入口，已保存的时间保持不变。

- 可为普通胶囊启用屏幕或外部窗口边缘磁吸；拖动进入触发范围后会显示吸附落点，松手完成吸附，吸附外部窗口后会随其移动。重新拖动、目标失效或关闭功能时自动解除，不影响贴边队列和主胶囊。

- 可把待办纸或笔记纸顶栏的「窗口绑定」按钮拖到其他软件窗口，松手后纸片会按自动或指定边缘贴合并随目标移动；拖动按钮可改绑，右键按钮可解除。展开纸片原有的贴边胶囊槽位会在绑定期间暂时让出，并在解除后自动恢复。
...
- 修复实验室窗口功能在快速关闭、目标窗口恢复、胶囊模式切换及虚拟桌面唤醒时可能遗留置底、半透明、旧吸附关系或错误显隐状态的问题。
