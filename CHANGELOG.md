...
- 高级模式开启后，设置窗口新增第四栏「实验室」；每项实验功能独立开关，关闭后不参与普通纸片和胶囊的运行。
...
- 可在实验室启用轻量待办提醒：待办拖拽柄左侧新增提醒键，支持默认间隔、10/30/60 分钟、当天傍晚、次日早晨和自定义时间；到期后使用系统托盘气泡单次提示。关闭实验开关时不显示按钮、不运行计时，已保存的提醒时间保持不变。

- 可为普通胶囊启用屏幕或外部窗口边缘磁吸；吸附外部窗口后会随其移动，重新拖动、目标失效或关闭功能时自动解除，不影响贴边队列和主胶囊。

- 可从展开纸片的右键菜单选择一个外部窗口进行系留，纸片会按自动或指定边缘贴合并随目标移动；重新拖动、目标失效、手动解除或关闭实验开关都会停止跟随。
