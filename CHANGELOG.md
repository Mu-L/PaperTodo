...
- 高级快捷键新增全部便签锁定、全部纸片透明、全部胶囊透明和当前焦点纸片透明；锁定覆盖插件便签，可选择是否允许点击矢量锁头解锁。快捷透明度独立于空闲半透明，并在设置值变化后立即同步。

- 全局快捷键可选择是否区分主键盘数字键与小键盘数字键；关闭区分时两者混合响应但不改写已保存绑定，切换模式遇到冲突时会保留现有快捷键。

- “失去焦点时自动收起纸片”新增严格收起：新建或显示后未使用纸片便转去其他操作时会立即收起，且不使用全局键鼠 Hook。实验室同时可让贴边胶囊与主胶囊不再保持置顶，展开纸片仍遵循自身置顶设置。
