...
- 连续浏览时，当前卡片在鼠标离开后继续保持；首次进入使用更宽松的移动意图门，已有卡片后的切换则根据鼠标速度、减速趋势和预计穿越时间自适应等待，快速扫过不会误开，停向目标时通常在下一帧切换。移动意图预测可在实验室关闭或选择低、中、高三档敏感度，中档为推荐平衡；移动距离与边缘范围会按当前显示缩放换算。离开整条边缘浏览区域仍不等待停稳直接收回。向队列上方浏览时，后续胶囊会按新卡片高度补位；向下浏览仍保留目标下方次序，避免跨项切换时发生视觉错序。完整纸片保留的边缘胶囊首次悬停、跨队列切换，以及在预览内操作过待办或链接后也不再偶尔失去自动展开响应。

- 预览卡片支持内部控件、滚动、背景点击打开完整纸片和独立关闭区；开始拖拽前会先恢复标准小胶囊及正常占位，现有跨屏、换边和回边拖拽继续只处理标准胶囊。
