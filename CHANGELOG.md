...
- 新增默认界面语言启动参数，可用 `--language`（或 `--lang`）指定中文、英文、日文或韩文，不覆盖本地纸片数据。
...
- 图片以增量方式存储在单个本地资产文件中，不会因图片增多而反复重写全部数据；单张图片元数据或内容损坏时只隔离该图片并在原位置显示损坏状态，不影响其他图片继续读取和写入；退出时自动清理未引用图片，外部打开笔记时安全导出临时文件。
- 批量插入图片整批成功后一次性写入，任一失败则整批回滚。
...
- 退出时可靠回收脚本胶囊的常驻 PowerShell 进程，单个窗口或步骤异常不会阻断完整退出。
