...
- 统一纸片左上角置顶入口为书钉图标：未置顶为空心书钉，已置顶为实心书钉，让用户更清楚察觉置顶功能。
- 优化关联笔记入口：关联笔记已显示时，再次点击不再重置位置，而是按贴边胶囊的再次点击设置进行收回或强调显示；关联入口会同步显示激活态。
- 贴边胶囊拖拽排序时的让位动画，减少排序过程中的生硬跳变。
- 适配 Windows 贴靠布局（半屏、四分之一屏、上下半屏），贴边时主动取消阴影以铺满分屏区域；隐藏后再次显示、收起为胶囊后再次展开会恢复到原贴靠区域。

**bug修复和边界修正**

- 修复窗口关闭时可能丢失最后 450ms 内编辑内容的问题，并避免删除纸片后立即退出时旧状态把已删除纸片带回。
- 修复贴边胶囊从激活态收回到贴边态时可能出现视觉硬切的问题。
- 优化多屏幕混合缩放环境下纸片收回为胶囊时的动画起点稳定性，避免收回动画开始前出现位置跳动。
- 优化贴边胶囊跨屏拖拽：横向拖动不再限定内侧方向，鼠标进入其他显示器时会直接进入跨屏拖拽。
- 修复贴边胶囊解锁为完整胶囊后跨屏拖拽时可能短暂偏离鼠标或闪烁的问题。
- 修复启用主胶囊时，断开副屏后副屏胶囊回落到主屏同侧可能出现两个主胶囊、且全部收起状态可能丢失的问题。
