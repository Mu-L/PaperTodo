...
- 插件协议升级到 1.4：宿主提供统一的原生下拉选择样式；插件设置、专注计时器和复盘记录池共用宿主选择器视觉，样例插件不再附带独立皮肤。
...
- 插件协议升级到 1.3：同主版本旧插件自动兼容；插件可按清单声明 Paper、Todo 与 Note 的读取、动态监听和受控写入。监听仅在纸片会话订阅期间启用，不引入纯后台插件；MCP 与插件共用强类型数据操作服务，插件不经过 MCP 服务器。
...
- 高级模式开启后，设置窗口新增第四栏「实验室」；每项实验功能独立开关，关闭后不参与普通纸片和胶囊的运行。实验室内容在原有设置窗口高度内滚动，不会改变窗口尺寸。

- 可分别设置失焦纸片与静止胶囊的透明度；普通胶囊和贴边胶囊会在悬停、激活或拖动时恢复实体，主胶囊不受影响。
