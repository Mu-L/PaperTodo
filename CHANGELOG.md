...
- 连续浏览时，当前卡片在鼠标离开后继续保持；首次进入使用更宽松的移动意图门，已有卡片后的切换则根据鼠标速度、减速趋势和预计穿越时间自适应等待，等待时间会从最后一次明显移动重新计算，并在真正展开前再次确认鼠标仍停向目标。移动意图预测可在实验室关闭或选择五档敏感度，中档为推荐平衡；移动距离与边缘范围会按当前显示缩放换算。鼠标位于队列外接矩形的空白区时，开启预测会在移动轨迹持续远离当前卡片和所有可浏览胶囊，或在空白区停留过久时按所选档位自动收回；关闭预测则不判断方向，连续处于空白区 3 秒后固定收回。当前卡片本体和其他真实胶囊区域仍会正常保活；离开整条边缘浏览区域仍不等待停稳直接收回。向队列上方浏览时，后续胶囊会按新卡片高度补位；向下浏览仍保留目标下方次序，避免跨项切换时发生视觉错序。完整纸片保留的边缘胶囊首次悬停、插件自绘胶囊在队列移动后重新移入、跨队列切换，以及在预览内操作过待办或链接后也不再偶尔失去自动展开响应；卡片和鼠标都静止时会跳过重复的队列命中计算，在高刷新率屏幕上减少持续开销而不限制动画帧率。

- 预览卡片支持内部控件、滚动、背景点击打开完整纸片和独立关闭区；背景点击只打开或置前完整纸片，保留当前浏览卡片直到鼠标真正离开。旧卡开始收回后会立即停止接收点击；拖拽任意队列胶囊前都会同步收回当前浏览卡片并恢复正常占位，现有跨屏、换边和回边拖拽继续只处理标准胶囊。
...
- 插件可为边缘快速浏览提供专属迷你界面：原生插件创建第二棵轻量 WPF 视图并与正文共享状态，Web 插件声明独立本地迷你入口。网页迷你界面首帧明确就绪前始终由放大的结构化胶囊接棒，初始化或脚本失败不会出现空白卡片；专注计时器可直接从迷你界面开始或暂停，时钟与云原神提供各自的轻量状态界面。
