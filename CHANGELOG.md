...
- 连续浏览时，当前卡片在鼠标离开后继续保持；首次进入使用更宽松的移动意图门，已有卡片后的切换则根据鼠标速度、减速趋势和预计穿越时间自适应等待，等待时间会从最后一次明显移动重新计算，并在真正展开前再次确认鼠标仍停向目标。移动意图预测可在实验室关闭或选择低、中、高三档敏感度，中档为推荐平衡；移动距离与边缘范围会按当前显示缩放换算。离开整条边缘浏览区域仍不等待停稳直接收回。向队列上方浏览时，后续胶囊会按新卡片高度补位；向下浏览仍保留目标下方次序，避免跨项切换时发生视觉错序。完整纸片保留的边缘胶囊首次悬停、插件自绘胶囊在队列移动后重新移入、跨队列切换，以及在预览内操作过待办或链接后也不再偶尔失去自动展开响应；卡片和鼠标都静止时会跳过重复的队列命中计算，在高刷新率屏幕上减少持续开销而不限制动画帧率。
