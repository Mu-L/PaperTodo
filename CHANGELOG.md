...
- 边缘胶囊与 slot 0 主胶囊改为按实际显示宽度绘制单侧圆角贴边标签，并在目标显示器坐标中一次提交完整物理像素矩形；拖出队列时改用独立悬浮拖拽窗口，贴边胶囊不再通过裁切、屏幕外溢出或复用悬浮布局形成外观，修复竖屏、混合缩放及拖拽归位后的裁切和形态残留。
...
- 拖动排序期间发生的队列重排和标题宽度刷新会在拖动或伸缩结束后统一应用，不再丢失刷新或打断关闭区伸缩。
