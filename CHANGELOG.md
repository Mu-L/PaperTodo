...
- 随附原生时钟、专注计时器、复盘记录池与 Web 时钟示例，均适配窄窗口和胶囊显示；专注计时器可从边缘迷你界面直接开始或暂停，复盘记录池按创建、完成、重新打开、删除与恢复事件保存记录，旧状态会自动迁移并保留有效数据。

- Web 插件仅向自身本地顶层页面注入 PaperTodo 桥接，外部链接交给默认浏览器；状态变化会即时保存，已加载插件文件发生变化或删除时会提示重新启动。
...
- 可从待办项左侧窄选区连续扫选多个完整项目，跨行拖选会继续按纵向位置扩展范围并自动滚动；多选后可一次复制、批量完成或恢复、右键删除全部，或拖动任一已选项的排序把手到底部垃圾区整组删除。
...
- 可为普通胶囊启用屏幕或外部窗口边缘磁吸：拖入触发范围会显示吸附落点，松手后平滑随目标移动并自动收进目标边缘，只留可操作的小头；鼠标移入滑出，移开后短暂延迟再收回，重新移入可取消收回。目标全屏或外侧空间不足时，小头保持在屏幕内侧；重新拖动、目标失效或关闭功能时自动解除，不影响贴边队列和主胶囊。

- 可把待办纸或笔记纸顶栏的「窗口绑定」按钮拖到其他软件窗口，松手后纸片按自动或指定边缘贴合并平滑跟随；可拖动改绑、右键解除，并沿目标边缘调整相对位置。窗口绑定可联动目标的最小化与激活：目标不可见时临时隐藏纸片或显示独立胶囊，目标恢复后无激活地重新显示纸片；绑定、恢复和解除过程保留纸片原有的展开、胶囊与可见性状态。
