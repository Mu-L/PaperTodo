...
- 边缘胶囊新增可交互的预览卡片形态：鼠标移入后以低延迟按内容请求的合理宽高展开，并真实占用队列空间；完整纸片已经展开且保留边缘胶囊时，也可以从边缘快速打开预览。

- 边缘胶囊队列明确不做自动分页、容量截断或自动翻页；无论队列多长都按完整顺序继续向下排列，超出当前显示器工作区的部分允许直接出屏。

- 同一边缘队列内的预览展开、收回、连续切换和队列补位改为使用按需创建的临时合成层，不再让每张纸常驻一整块屏幕高度的透明区域，也不再为每一帧反复移动多个系统窗口；高刷新率屏幕上的补位更稳定，连续切换会在最后时刻接续同一合成帧，不再因准备下一张卡片而短暂停住或裁切旧卡，展开和收回也不会在端点额外停留或因新旧画面重叠而闪一帧。过渡完成后仍由原胶囊接管完整内容和输入，不支持的场景会自动使用原有路径。

- 连续浏览时，当前卡片在鼠标离开后继续保持；首次真实命中会在下一次绘制前立即展开，已有卡片后的切换只要求指针在同一目标内连续停留 32 毫秒，不再因目标内正常移动反复重新计时，并会利用这段等待提前准备下一张画面。移动意图预测只能继续延后或取消切换，不会缩短这段基础等待。预测可在实验室关闭或选择五档敏感度，中档为推荐平衡；移动距离会按鼠标所在显示器的缩放换算。鼠标位于胶囊队列外接矩形的空白区时，开启预测仅在轨迹明确朝向某个可浏览胶囊时继续保留，否则按所选档位在约 0.2～0.8 秒后收回；关闭预测则不分析轨迹，固定等待 1 秒。当前卡片本体和其他真实胶囊区域仍会正常保活；一旦离开外接矩形，两种模式都立即收回，预测不会覆盖这条边界。向队列上方浏览时，后续胶囊会按新卡片高度补位；向下浏览仍保留目标下方次序，避免跨项切换时发生视觉错序。快速连续切换会按同一帧更新整条队列，保持卡片间距稳定；不同显示器上的队列不会互相截断动画，并且只保留当前卡片与一张正在收回的卡片。完整纸片保留的边缘胶囊首次悬停、插件自绘胶囊在队列移动后重新移入、跨队列切换，以及在预览内操作过待办或链接后也不再偶尔失去自动展开响应；卡片和鼠标都静止时会跳过重复的队列命中计算，在高刷新率屏幕上减少持续开销而不限制动画帧率。

- 预览卡片支持内部控件、滚动、背景点击打开完整纸片和独立关闭区；背景点击只打开或置前完整纸片，保留当前浏览卡片直到鼠标真正离开。旧卡开始收回后会立即停止接收点击；拖拽任意队列胶囊前都会收回当前浏览卡片并恢复正常占位，从高预览卡开始拖动时也不会再让抓取位置突然偏移。拖出的悬浮胶囊会提前准备并跨手势复用，浮动期间隐藏的原槽位保持紧凑，也不会在悬浮胶囊下方再次展开另一张预览；现有跨屏、换边和回边拖拽继续只处理标准胶囊。

- 待办纸的边缘预览会按当前内容实时显示简化待办列表，可滚动浏览并直接完成或恢复待办；超长列表只展示靠前的代表项目，完整内容仍从纸片打开，预览不提供编辑、新增、删除或排序。

- Markdown 笔记的边缘预览直接读取当前编辑文本，并在有限内容范围内轻量渲染标题、强调、删除线、行内代码、引用、列表、任务项、链接、分隔线、代码块和图片占位；卡片尺寸在本次展开期间保持稳定，标题和正文变化会即时合并刷新。

- 内置待办、Markdown 与插件卡片统一通过同一个内容入口提供尺寸和实时内容；空待办与空笔记默认缩小为 `130 × 120 DIP`，完整卡片允许的绝对下限调整为 `120 × 90 DIP`。

**插件协议 1.8**

- 插件协议升级到 1.8，宿主只接受 1.8 协议插件。插件仍依附于一张笔记纸，但所属纸片、展开正文和整个工作区的能力范围保持明确，正式标题、展开态运行时标题与胶囊表现互相独立。

- 插件可为自己的胶囊提交最多三个任意顺序的标准组件，包括文字、图标、状态点、进度环和进度条；胶囊既可指定固定内容宽度，也可由 PaperTodo 按当前组件自动适配，随附的时钟、专注计时器、复盘记录和云原神插件不再为短状态保留大段空白。PaperTodo 继续统一负责固定高度、外壳、关闭区、贴边和交互。

- 原生插件可在固定高度的胶囊内容区自由绘制动态图形，普通胶囊与贴边胶囊分别创建独立纯 WPF 视图；外壳和全部鼠标交互仍由 PaperTodo 掌控，自绘失败自动退回标准组件模板。WebView2、原生子窗口和已经挂载的控件不再作为偶然可用的自绘胶囊内容接受。

- 插件可为边缘快速浏览提供专属迷你界面：原生插件创建第二棵轻量 WPF 视图并与正文共享状态，Web 插件声明独立本地迷你入口。宿主会先展开现有胶囊内容的轻量回退，再创建插件界面，耗时的界面创建不再阻塞占位卡片出现；网页迷你界面只接受当前文档的明确就绪信号，并在完成一次真实绘制前始终由回退内容接棒，初始化、脚本失败或打开外部链接都不会把健康页面错误替换为空白卡片；专注计时器可直接从迷你界面开始或暂停，时钟与云原神提供各自的轻量状态界面。

- 未提供专属迷你界面的插件不再只显示纯标题：宿主依次使用自绘胶囊的实时放大镜像、重新排版的标准胶囊组件和纯文字回退。完全由 WPF 构成的原生插件还可明确允许唯一正文视图迁移；首次浏览可显示真实界面，打开纸片后通过内存截图接棒，以后先显示旧截图并只刷新一次，不持续截图，也不为 WebView2 创建第二套重型界面。

- 允许正文迁移的纯 WPF 插件会在纸片折叠并贴边后的空闲阶段，提前按浏览卡片最终尺寸完成首次排版；真正移入时直接复用这棵已排好的界面，减少第一次展开过程中临时搬运和重排带来的卡顿。

**设置、插件与交互**

- 设置页新增界面语言下拉选择，可选跟随系统、简体中文、English、日本语或韩语；选择会保存，重启 PaperTodo 后生效。

- 实验室取消小类卡片边框，改用大类分割线；“快捷键置底”更名为“高级快捷键”。插件“更多设置”固定在设置尾部，并修复 Web 时钟设置卡右侧描边裁切。

- 高级快捷键新增全部便签锁定、全部纸片透明、全部胶囊透明和当前焦点纸片透明；全部胶囊透明同时覆盖主胶囊和之后才创建的胶囊，锁定覆盖插件便签并延续到之后才创建的贴边胶囊，可选择是否允许点击矢量锁头解锁。快捷透明度独立于空闲半透明，并在设置值变化后立即同步。

- 全局快捷键可选择是否区分主键盘数字键与小键盘数字键；关闭区分时两者混合响应但不改写已保存绑定，切换模式遇到冲突时会保留现有快捷键。

- “失去焦点时自动收起纸片”新增严格收起：新建或显示后未使用纸片便转去其他操作时会立即收起，且不使用全局键鼠 Hook；用于打开纸片的快捷键释放不会被误认为下一次操作。实验室同时可让贴边胶囊与主胶囊不再保持置顶，展开纸片仍遵循自身置顶设置。

- 修复 Unity 等无边框全屏程序前台运行时，主胶囊、边缘胶囊及悬浮提示仍可能压在画面上方的问题；退出全屏后照常恢复置顶。

- 修复把边缘胶囊拖到另一台显示器后，点击展开时纸片仍可能回到原显示器的问题。

- 修复设置页选择菜单未完整跟随当前主题、原生插件选择器在选中后显示内部英文值和选项点击区域不完整的问题；待办跨行多选在文字选区之外同时保留每条已选待办的深色整行预选底色，并且只绘制原有文字上方的选区背景，不再把文字改成白色；即使尚未进入第二行就先拖出纸片，也会继续按纵向位置升级跨选、扩展范围并自动滚动。

- 调整纸片尺寸时，原生插件继续实时重排，但会合并连续尺寸事件中的重复裁切，只有跨过窄布局断点时才修改响应式属性；复盘记录池只布局当前可见的记录，降低拖动窗口边框时的卡顿。

- 修复待办跨行扫选时鼠标捕获状态提前清空，导致其他纸片暂时无法操作的问题。

- 设置页的普通开关、实验室依赖项、高级快捷键录制与开关，以及插件运行状态改为只更新所在区域；插件状态变化不再收起“更多设置”或打断正在编辑的设置项。

- 插件协议升级到 1.4：宿主提供统一的原生下拉选择样式；插件设置、专注计时器和复盘记录池共用宿主选择器视觉，随附插件统一声明 1.4，样例插件不再附带独立皮肤。宿主支持范围明确收紧为 1.2–1.4，不再加载 1.0/1.1 插件。

- 待办文字拖选进入其他项目时可直接升级为整项多选；文件快启名称跟随关联纸片名称与长标题设置，并可在短标题模式仅显示扩展名。高级模式新增项以连续浅色块和菱形标记区分；视觉页可调边缘胶囊间距。插件页显示运行状态点、紧凑版本信息与响应式“更多设置”，并修复插件下拉框和 Web 时钟显示。被待办关联的笔记纸或待办纸均可按设置退出胶囊队列。

- 边缘胶囊较窄、标准、较宽三档间距调整为 0、4、8 DIP，视觉差异更明显。

- 调整展开纸片、普通浮动胶囊、拖出的浮动边缘胶囊和主/贴边胶囊的阴影层次，使不同形态更易区分。

**待办**

- 待办项可拖入一个文件或文件夹作为快启路径；路径只保存链接，不复制内容，并与纸片关联互斥。路径失效时显示警告色感叹号，可从右键菜单打开、打开所在位置或解除关联。

- 新增、删除、排序、完成、提醒和快启关联只重建发生变化的待办行；仅挂有路径、提醒或纸片关联的空文字待办不会再被退格或删除空纸片流程当作空项静默移除。

- 可从待办项左侧窄选区连续扫选多个完整项目，按 Ctrl+C 一次复制；多选后点击任一已选项左侧勾选框可批量完成或恢复，右键可删除全部，拖动任一已选项的排序把手到底部垃圾区可整组删除。

**构建与发布**

- 发布工作流改为完整检出 Git 历史与子模块，避免 wpf-notifyicon 的 Nerdbank.GitVersioning 在浅克隆中无法计算版本高度。

**插件与正文扩展**

- 修复插件设置中的选择项打开后异常退出。

- 专注计时器与复盘记录池的下拉框统一为纸片风格。

- 修复 Web 时钟无法正常显示的问题。

- 插件清单可声明一个由布尔设置控制的启动纸片；PaperTodo 完成常规恢复后会在界面空闲时统一创建或恢复对应插件纸，不再允许插件自定义启动延迟，并通过「插件 ID + 实例键」保证同一实例不会重复生成。首个示例为 Web 时钟的“启动后自动创建时钟胶囊”选项。

- 插件运行时标题会保留最后一次有效值；隐藏或尚未重新加载的插件纸片，也能在关联待办和胶囊中显示上次的动态标题。

- 官方原生时钟、专注计时器、复盘记录池与 Web 时钟改进窄窗口布局；插件正文仍由纸片边界主动裁切，但关键操作会换行或缩放，不再永久落到可视区域之外。

- 待办关联从“关联笔记”泛化为“关联纸片”，可指向笔记、插件正文或其他待办纸；普通拖拽、MCP 与插件 API 使用相同校验，删除目标纸片会统一解除关联。

- 插件待办快照和变化事件同时公开关联纸片与快启路径；路径新增、删除或更换会产生可辨别的待办事件。

- 复盘记录池改为保存创建、完成、重新打开、删除与恢复事件，并合并短时间内的界面刷新和原子写盘；旧版快照会无损迁移为事件记录。专注计时器状态升级到 v2，旧计时进度和累计轮数保持不变。

- 插件协议升级到 1.3，并保留对 1.2 插件的加载支持；插件可按清单声明 Paper、Todo 与 Note 的读取、动态监听和受控写入。监听仅在纸片会话订阅期间启用，不引入纯后台插件；MCP 与插件共用强类型数据操作服务，插件不经过 MCP 服务器。

- 插件协议 1.2 的设置默认值改为严格校验；纸片删除后仅在主数据保存成功时清理插件状态。

- Web 插件恢复完整 WebView2 导航、远程框架、弹窗与权限请求能力；PaperTodo 桥接仍仅注入插件本地顶层页面。常规网络下载交给默认浏览器，会话内生成的下载继续由 WebView2 处理。

- 笔记正文支持内置 Markdown、可信原生 WPF 与本地 Web 双插件；设置新增「插件」页，右键笔记可切换正文。每个插件使用独立的 `plugins/<插件 ID>/` 目录，清单、入口、依赖、资源和该插件的 `.runtime` 数据放在一起。插件不提供沙箱，Web 侧仅保留轻度防误用限制，请只安装可信插件。

- 插件协议升级到 1.1：插件页顶部显示当前宿主协议，每张插件卡显示插件版本、协议版本与状态数据版本；协议版本改为严格的带引号字符串，原生 DLL 的协议和后台运行要求必须与清单一致。

- 插件可提供不写入数据文件的运行时显示标题，纸片顶栏与胶囊保持同步；需要折叠后继续更新的插件必须显式声明后台更新要求，冷启动使用独立隐藏运行面而不会移动真实纸片窗口。插件正文恢复与纸片一致的底部圆角。

- 插件可动态声明占用 Esc 与正文右键菜单。占用期间 PaperTodo 主动避让，原始输入继续交给插件；标题栏和边框仍保留 PaperTodo 菜单，会话切换、失败或销毁时自动释放。

- Web 插件限制在自身虚拟源中运行，外链交给默认浏览器，并拦截远程框架、弹窗、下载与权限请求；状态变化必须即时保存，版本迁移、进程失败和缺失插件都有明确回退，无法解析的已存状态会原样保留而不会被空数据覆盖。原生插件每个正文会话使用新的工厂实例，未被纸片使用时不会在启动阶段加载 DLL，已加载 DLL 的变化或删除会提示重启。

- 内置 Markdown 的编辑器、焦点、预览、图片与重建状态迁入独立正文会话，PaperWindow 只保留成熟交互方法的过渡转发，正文根节点在渲染模式重建时保持稳定；Markdown 链接和裸 `http://` / `https://` 地址均可点击打开，行内及围栏代码中的示例地址保持为普通文本，强调标记不会混入实际网址。

**实验室功能**

- 新增本地 MCP 接口：由同一个 `PaperTodo.exe` 以 `--mcp` 模式提供，无需额外分发 MCP 程序。启用后默认只读，并可分别允许空白/追加写入、完整写入和直接删除；空白/追加写入可新建纸片、向已有待办纸新增待办及填充或追加笔记。直接删除默认关闭，开启即视为预授权，不再逐次弹窗确认；设置页可分别复制 JSON 连接配置与指导 AI 正确使用 PaperTodo 工具的 Skill 文本。

- 高级模式开启后，设置窗口新增第四栏「实验室」；内容改为整页双列，并把失焦/静置行为与窗口绑定/可见性联动/边缘磁吸分别收束为两组。每项实验功能独立开关，关闭后不参与普通纸片和胶囊的运行；实验室仍在原有设置窗口高度内滚动，不会改变窗口尺寸。

- 可分别设置失焦纸片与静止胶囊的透明度，并可让展开纸片在失去焦点后自动收起；拖拽、标题编辑、菜单或下拉栏交互不会误触发自动收起。普通胶囊和贴边胶囊默认在悬停、激活或拖动时恢复实体，也可选择激活时继续透明或把相同透明度覆盖到主胶囊。

- 可在纸片失去焦点时仅隐藏顶栏操作按钮，或让普通浮动纸片从顶部真实收短整条标题栏并保持正文与底边位置不动；重新激活及菜单或拖动交互时向上恢复，最大化、Snap 和深胶囊槽位继续保留完整标题栏。

- 可设置全局快捷键，将当前纸片切换为鼠标点击穿透并置底；再次按键、关闭实验开关、隐藏纸片或使用托盘恢复入口都会退出该状态。

- 可用另一组全局快捷键将全部纸片、普通胶囊、贴边胶囊和主胶囊统一切换为点击穿透并置底；拖拽胶囊期间不会切换，托盘恢复入口可统一解锁。

- 可在实验室启用轻量待办提醒：支持从待办行按钮或右键菜单选择默认间隔、10/30/60 分钟、当天傍晚、次日早晨和自定义时间；自定义时间使用紧凑的纸片风格输入框，可直接输入日期和时间，通过今天/明天快速选日，并按住加减按钮连续微调。设置后在待办行显示可点击的简洁倒计时，可随时修改或取消；到期时会打开并定位对应待办，同时通过系统托盘单次提示。系统时间调整、休眠恢复和解锁后会立即重新检查，平时仍以最长一分钟的轻量轮询兜底。可单独隐藏铃铛按钮、仅保留右键入口；关闭提醒功能时停止计时并隐藏入口，已保存的时间保持不变。

- 可为普通胶囊启用屏幕或外部窗口边缘磁吸；拖动进入触发范围后会显示吸附落点，松手完成吸附，只有仍有可见区域的常规软件主窗口会参与吸附。吸附外部窗口后会平滑随其移动并自动收进目标边缘，只留一个可操作的小头；鼠标移入时稳定滑出，移开后自动收回，不保留额外激发状态。目标全屏或外侧空间不足时，小头会留在屏幕内侧；同时开启窗口绑定后，打开胶囊会无缝转为对同一窗口的绑定并继续跟随。重新拖动、目标失效或关闭功能时自动解除，不影响贴边队列和主胶囊。

- 可把待办纸或笔记纸顶栏的「窗口绑定」按钮拖到其他软件窗口，松手后纸片会按自动或指定边缘贴合并平滑随目标移动；按钮会明确显示未绑定与已绑定状态，拖动按钮可改绑，右键按钮可解除。绑定后沿目标边缘轻拖标题栏可调整位置，并留出更大的向外拖动距离，避免调整位置时误解除。目标靠近屏幕边缘时纸片即使部分超出屏幕也会继续跟随，并保留可操作的顶栏区域。展开纸片原有的贴边胶囊槽位会在绑定期间暂时让出，并在解除后自动恢复。

- 窗口绑定可选联动目标的最小化与激活：目标不可见时临时隐藏纸片或显示独立胶囊，目标恢复后无激活地重新显示纸片；该过程不改写纸片原有的展开、胶囊或可见性数据。

- 可在 Windows 10 及以上系统启用当前虚拟桌面唤醒：主动显示纸片、从托盘唤醒或打开普通/贴边胶囊时，可将 PaperTodo 自有窗口移到当前桌面；不会切换系统桌面或移动外部软件，关闭开关后立即释放接口。该实验与「从窗口切换中隐藏纸片」互斥，默认全桌面可见模式下不会接入虚拟桌面接口。

**优化**

- 优化高刷新率和高 DPI 下的窗口跟随与胶囊动画：外部窗口跟随按实际经过时间判断稳定，贴边胶囊按 WPF 实际渲染帧更新并以真实时间限制宿主应用重试；外部窗口候选的最小尺寸按其所在显示器 DPI 计算，窗口绑定按纸片真实顶栏拖拽区域保留可操作空间。

- **优化窄纸片顶栏按钮与标题防重叠响应式布局**：基于顶栏实际可用宽度和稳定尺寸进行分阶段碰撞判定，碰撞时优先收起右侧功能按钮组并始终保留右侧关闭/折叠按钮，空间极窄时再按需收起标题，避免窄尺寸下顶栏元素重叠或反复闪烁；支持更窄的纸片最小宽度。

- **优化顶栏图标与纸片右键菜单结构**：将准星/关联完成、新建纸片及折叠图标改为按设备像素对齐的轻量矢量绘制，根据奇偶线宽引入设备物理像素中线校准（`AxisCenter`），并锁定到屏幕物理像素全屏坐标（`DevicePixelOrigin`），彻底解决非整数 DPI 缩放下的像素半缝隙发虚；准星、加号、减号及设置矢量的人工线条统一采用可持续扩展的 DPI 线宽阶梯（`StrokePixelsForDpi`），100%～150% 统一使用 1 个物理像素精细呈现、175% 起使用 2 个物理像素，并在 275%、375% 等超高缩放下按比例递增；斜向的对号、铅笔、关闭叉与托盘纸片类型图标继续沿用原系统字形轮廓，避免改变原有造型和视觉重量；外部编辑器扩展名继续使用原字体及既有位置补偿；当纸片绑定外部窗口或被其他待办关联时，关联图标由四向轻微出头的细准星切换为细圆环内的大圆点，保持更轻的视觉重量；托盘新建图标的加号角标适度放大并锁定到屏幕物理像素中心；新增可复用的矢量设置齿轮图标；简化并统一纸片右键菜单分组，顶部引入「快捷」分组头并提供新建及设置入口；任何状态下均保留「设置」菜单项；胶囊恢复统一显示为「展开为纸片」；「删除纸片」高亮危险提示状态；笔记纸右键菜单中增加使用外部 MD 编辑器打开的快捷选项。

- **优化右键菜单多交互面实时同步刷新**：统一纸片主体、胶囊图标区、关闭区、贴边宿主及 Markdown 预览区的右键菜单刷新逻辑，在折叠、切换模式或窗口状态变化时同步重置所有可用交互面的右键菜单实例，确保状态相关菜单项始终保持最新且不滞后。

**bug修复和边界修正**

- 修复笔记正文处于编辑状态时，第一次在纸片顶栏打开右键菜单可能闪现后立即关闭的问题；标题提交或状态变化恰逢菜单打开时，会等当前菜单关闭后再同步刷新其他交互面的菜单。

- 优化全屏避让：取消边缘胶囊的高频置顶保护，改为前台窗口事件触发、1 秒轻量检查和 5 秒全局兜底；关闭避让后不再保留相关监听与定时检查。

- 修复程序重启后，直接从未展开的贴边胶囊右键删除非空纸片时，删除确认框可能导致异常退出的问题。

- 修复任务栏托盘菜单在程序首次打开或 Windows 显示缩放切换后的第一次右键中，可能被夹到屏幕右下角或闪现后将焦点退回纸片的问题；鼠标菜单由 WPF 按实时鼠标位置和当前显示器 DPI 放置，并在进入菜单模式前清理纸片遗留的跨进程激活状态；托盘样式菜单按实际 Popup 所在显示器工作区限制最大高度，避免副屏、竖屏或不同缩放组合下高度不合适。

- 修复笔记进入并退出编辑状态后，正文右键菜单可能恢复旧标题或旧操作状态的问题。

- 修复实验室窗口功能在快速关闭、目标窗口恢复、胶囊模式切换及虚拟桌面唤醒时可能遗留置底、半透明、旧吸附关系或错误显隐状态的问题；实验室功能全部关闭时不再干扰原有胶囊贴边队列，贴边胶囊不会只剩主胶囊。

- 优化待办提醒可靠性与触发反馈：打开目标纸片或弹出托盘通知成功前保留提醒数据，交付均失败时自动重试；成功触发后通过纸片或贴边胶囊动效及待办行高亮持续标记，直到用户编辑或完成该项后确认清除；MCP 客户端无响应等待上限缩短为 10 秒。

- 优化胶囊边缘吸附与窗口系留防抖：鼠标离开磁吸胶囊后增加 180ms 延迟收回机制并可随时再次取消，防止边缘悬停抖动；优化外部窗口枚举节流，没有实际外部窗口吸附或绑定时不再安装系统级事件监听；并将沿边拖离解绑阈值调大至 72 DIP，提升拖拽操作稳定性。
...
- **笔记图片缓存**：同一张笔记图片只保留一份解码缓存，并按最近使用淘汰：最多约 20 张或约 50 MB 解码像素（先到先限），**正在显示的图片不受该上限限制**。

- 按DPI 和实际显示宽度精确解码，优化图片显示清晰度。

**bug修复和边界修正**

- 纸片不再响应 Windows 最小化命令：普通状态下按 Win+↓ 不会最小化；从最大化恢复到普通窗口及最大化操作仍保持系统默认行为。

- 修复启动时忽略已隐藏纸片/胶囊的 `isVisible` 配置，隐藏的胶囊重新打开后仍会显示的问题；隐藏状态会按数据文件正确恢复，托盘「显示全部」仍可一次找回，并使用与启动一致的批量恢复流程平稳重建贴边标签。
