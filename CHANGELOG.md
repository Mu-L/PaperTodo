...
- 连续浏览时，当前卡片在鼠标离开后继续保持；首次真实命中会在下一次绘制前立即展开，已有卡片后的切换则必须先在目标内稳定 50 毫秒，移动意图预测只能继续延后或取消切换，不会缩短这段基础等待。预测可在实验室关闭或选择五档敏感度，中档为推荐平衡；移动距离与边缘范围会按鼠标所在显示器的缩放换算。鼠标位于队列外接矩形的空白区时，开启预测会在移动轨迹持续远离当前卡片和所有可浏览胶囊，或在空白区停留过久时按所选档位自动收回；关闭预测则不判断方向，连续处于空白区 3 秒后固定收回。当前卡片本体和其他真实胶囊区域仍会正常保活；离开整条边缘浏览区域仍不等待停稳直接收回。向队列上方浏览时，后续胶囊会按新卡片高度补位；向下浏览仍保留目标下方次序，避免跨项切换时发生视觉错序。快速连续切换会按同一帧更新整条队列，保持卡片间距稳定；不同显示器上的队列不会互相截断动画，并且只保留当前卡片与一张正在收回的卡片。完整纸片保留的边缘胶囊首次悬停、插件自绘胶囊在队列移动后重新移入、跨队列切换，以及在预览内操作过待办或链接后也不再偶尔失去自动展开响应；卡片和鼠标都静止时会跳过重复的队列命中计算，在高刷新率屏幕上减少持续开销而不限制动画帧率。
...
- 边缘胶囊数量超过当前显示器可安全容纳的范围时，会自动按页显示并在队列页头提供前后翻页，不再把末尾胶囊压在同一个位置；靠近任务栏展开高预览时，当前页会整体上移并继续保持原有卡片间距，不会让预览正文或后续胶囊落到工作区外。
...
- 插件可为边缘快速浏览提供专属迷你界面：原生插件创建第二棵轻量 WPF 视图并与正文共享状态，Web 插件声明独立本地迷你入口。宿主会先开始展开轻量占位内容，再创建插件界面，耗时的界面创建不再阻塞占位卡片出现；网页迷你界面只接受当前文档的明确就绪信号，并在完成一次真实绘制前始终由回退内容接棒，初始化、脚本失败或打开外部链接都不会把健康页面错误替换为空白卡片；专注计时器可直接从迷你界面开始或暂停，时钟与云原神提供各自的轻量状态界面。
