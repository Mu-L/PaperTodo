...
- 边缘胶囊新增可交互的预览卡片形态：第一次悬停短暂停留后按内容请求的合理宽高展开，并真实占用队列空间；完整纸片已经展开且保留边缘胶囊时，也可以从边缘快速打开预览。
...
- 待办纸的边缘预览会按当前内容实时显示简化待办列表，可滚动浏览并直接完成或恢复待办；预览只承担浏览和轻操作，不提供编辑、新增、删除或排序。

- Markdown 笔记的边缘预览直接读取当前编辑文本并轻量渲染标题、强调、删除线、行内代码、引用、列表、任务项、链接、分隔线、代码块和图片占位；卡片尺寸在本次展开期间保持稳定，内容变化会原位刷新。
