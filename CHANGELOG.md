...
- **优化顶栏图标与纸片右键菜单结构**：更新窗口绑定与关联按钮图标为 `⌖` / `○`，当纸片绑定外部窗口或被其他待办关联时显示为 `○` 并用主题激活色加粗高亮；简化并统一纸片右键菜单分组，顶部引入「快捷」分组头并提供新建及设置入口；任何状态下均保留「设置」菜单项；胶囊恢复统一显示为「展开为纸片」；「删除纸片」高亮危险提示状态；笔记纸右键菜单中增加使用外部 MD 编辑器打开的快捷选项。
