...
- 插件协议升级到 1.2：插件页改为紧凑卡片布局，可由宿主绘制布尔、文本、数字和下拉设置；每个插件的全局设置与纸片状态独立存入 `plugins/data/<插件 ID>.json`，单张状态保存限制为 1 MiB，不再写入主数据文件。数据读取失败时保留原文件并使用唯一恢复文件，删除纸片会同步清理插件状态；旧插件与旧状态默认不兼容，可由用户临时开启同主版本旧协议的强制加载。
