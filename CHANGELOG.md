...
- **多屏贴边胶囊位置**：评估是否允许选择贴边胶囊所在屏幕与左右边缘。需要兼顾双屏边界不裁剪导致尾部露到另一块屏幕的问题、用户对主屏 / 副屏固定位置的预期、显示器插拔后的回退规则，以及是否把它做成常态设置还是通过拖到屏幕边缘来记忆。
...
- **托盘菜单点击修复**：修复托盘右键菜单偶尔第一次点击无效的问题。
- **全屏置顶策略**：设置中新增「避让 / 不避让」两档，默认避让外部全屏窗口，并改善浏览器视频全屏播放时的自动识别，减少进入全屏后仍需点击画面的情况。
- **纸片创建位置保护**：新建纸片会避开任务栏工作区，避免在屏幕边缘创建后无法点击。
- **贴边胶囊起始高度**：支持拖动「收起全部」主胶囊调整贴边胶囊栈的起始高度。
- **多屏贴边显示修复**：贴边胶囊改为在窗口内部裁剪尾部，避免双屏相邻边界上尾部露到另一块屏幕。
- **贴边主胶囊显示修复**：修复「收起」主胶囊右侧被过度裁切的问题，并清理贴边胶囊裁剪时可能出现的异常阴影。
- **贴边胶囊动画修复**：修复激发态胶囊收回时先裁切后移动导致短暂空区的问题，优化激发态外移距离，并让「收起全部」回收时只沿垂直方向移动。
- **贴边胶囊展开修复**：修复从贴边胶囊进入纸片时，主窗口可能停留在胶囊尺寸导致标题控件单独露出的问题。
