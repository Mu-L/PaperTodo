...
拖动边缘胶囊时当前胶囊划过其他胶囊时层级应该更高

右键边缘胶囊不应该收起

右键边缘胶囊时，有时候右键菜单不在最顶层，不稳定复现

有时候莫名其妙退出不完全残留进程，不稳定复现

拖拽过小的时候错误裁切

开启胶囊模式下esc会缩小胶囊

快捷键面板，和设置互相内置跳转，任务栏启动

快捷键支持ctrl+alt+1启动边缘序列1胶囊

支持自定义设置最长边缘胶囊长度

补齐readme

1. 版本 badge：v2.1 → v2.2
1. 置顶按钮：类型图标 → 图钉图标
1. 新增 Windows 分屏贴靠说明
...
右键插入图片无效，按理说应该弹出一个windows文件选择器吧。

图片光标的位置错误，在图片后面很多的位置
...
- 笔记支持粘贴、拖入或从菜单插入图片，编辑态和浏览态都会按纸片宽度完整显示图片；点击图片、连续插入或拖放图片后，光标会稳定落在图片下方继续编辑。
- 图片会显示在对应 Markdown 图片语法下方，新插入图片默认使用 `![image|100%](i:001)` 这类短编号语法，并支持通过 `![image|宽x高, 75%](...)` 或 `{width=75%}` 调整显示大小；复制、粘贴和撤销不会夹带隐藏字符。
- 图片随本地数据保存，删除图片引用后仍可在本次编辑中撤销，退出时会清理不再引用的图片数据；外部打开笔记时会同步导出临时图片文件，方便普通 Markdown 编辑器显示。
