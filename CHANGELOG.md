...
- **边缘胶囊实时预览**：鼠标悬停在边缘胶囊上即可展开轻量、可交互的预览卡片，无需打开完整纸片即可即时浏览内容。
  - **待办预览**：实时显示简化待办列表，支持直接滚动浏览、勾选完成或撤销，点击卡片背景可直接呼出完整纸片。
  - **笔记预览**：轻量渲染 Markdown 笔记正文，支持标题、强调、删除线、代码块、列表与图片占位。
  - **流畅接续与意图预测**：内置可调节的鼠标移动意图预测，在胶囊间连续浏览时减少误切换，离开可浏览区域后自动收回。
  - **统一尺寸与拖拽保护**：拖动胶囊排序或移出时自动收起预览；空待办与空笔记默认以更紧凑的卡片尺寸展示。

**插件与正文扩展（协议 2.1）**

- **正文插件生态**：笔记纸支持切换为本地 Web 插件或原生 WPF 插件，设置中新增独立「插件」管理页；宿主使用 2.1 协议；插件不提供安全沙箱，仅建议安装可信来源的插件。
  - **基础插件能力**：插件可按需声明 Paper/Todo/Note 数据读写权限，支持自定义顶栏操作按钮、注册全局快捷键、控制所属纸片显隐/折叠展开/激活、共享 App 后台运行时以及独立配置面板，并支持按设置自动创建或恢复专属启动纸片。
  - **协议 2.1 宿主原生扩展**：插件可为待办项发布由 PaperTodo 统一渲染的行内/右键操作，并可向任意现有纸片发布不可点击的顶栏标签；触发待办操作时会收到该事项的当前状态快照。
  - **输入接管与生命周期**：正文插件可声明独占接管 Esc 键与正文右键菜单，退出插件会话后自动归还宿主。
  - **自绘胶囊与专属迷你视图**：插件支持自定义折叠胶囊（图标、文字、进度环/条或纯 WPF 自绘图形），并可为边缘悬停提供专属迷你卡片视图。
  - **Web 插件外链处理**：Web 插件内的外部网页链接与常规下载会自动转交系统默认浏览器处理。
  - **示例插件**：仓库提供原生时钟、专注计时器、复盘记录池、云原神、Web 时钟与 Web 顶栏等示例，适配窄窗口、胶囊与迷你视图。
...
- **Markdown 解析与显示一致性重构**：笔记的标题、引用、列表、代码块、链接、简单 HTML、转义与图片代码区判断统一到同一套 Markdown 语义，编辑与浏览继续复用原正文，减少复杂嵌套时样式、点击和列表编辑不一致。
