...
- 连续浏览时，当前卡片在鼠标离开后继续保持；首次进入使用更宽松的移动意图门，已有卡片后的切换则根据鼠标速度、减速趋势和预计穿越时间自适应等待，快速扫过不会误开，停向目标时通常在下一帧切换。离开整条边缘浏览区域仍不等待停稳直接收回。向队列上方浏览时，后续胶囊会按新卡片高度补位；向下浏览仍保留目标下方次序，避免跨项切换时发生视觉错序。
