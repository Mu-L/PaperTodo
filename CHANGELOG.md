...
- 设置窗口内新增固定的“通用 / 快捷键”页签，托盘统一从“设置”进入；支持为显示全部、隐藏全部、切换显隐、新建待办、新建笔记和退出六种全局操作录入、清空或恢复快捷键。
...
- 图片集中保存在单个本地资产文件中并按次增量写入，不再随图片数量增加而反复重写全部图片数据；删除图片引用后仍可在本次编辑中撤销，退出时会清理不再引用的图片，外部打开笔记时会安全导出临时图片文件。
