...
- 同一边缘队列内的预览展开、收回、连续切换和队列补位保持连续动画；快速切换时旧卡与新卡平滑接续，高刷新率屏幕上保持内容完整、间距稳定，过渡完成后仍由原胶囊接管完整内容和输入。

- 连续浏览时，当前卡片在鼠标离开后继续保持；首次真实命中会在下一次绘制前立即展开，已有卡片后的切换只要求指针在同一目标内连续停留 32 毫秒。移动意图预测可在实验室关闭或选择五档敏感度，中档为推荐平衡；预测只会延后或取消切换，不会提前放行。鼠标位于队列外接矩形的空白区时，开启预测仅在轨迹明确朝向可浏览胶囊时继续保留，否则按档位在约 0.2～0.8 秒后收回；关闭预测则固定等待 1 秒；离开外接矩形时立即收回。向上、向下连续浏览会分别保持合适的锚点和队列补位，快速扫过多张卡片时仍维持稳定顺序。

- 预览卡片支持内部控件、滚动、背景点击打开完整纸片和独立关闭区；背景点击只打开或置前完整纸片，并保留当前预览直到鼠标真正离开。拖拽任意队列胶囊前会收回当前预览并恢复标准占位，抓取位置保持稳定；拖出的悬浮胶囊在拖动期间独立显示，现有跨屏、换边和回边拖拽继续按标准胶囊处理。
...
- 内置待办、Markdown 与插件卡片统一使用同一套预览尺寸规则；空待办与空笔记默认缩小为 `130 × 120 DIP`，完整卡片允许的绝对下限调整为 `120 × 90 DIP`。

**插件与正文扩展（协议 1.8）**

- 笔记正文支持内置 Markdown、可信原生 WPF 与本地 Web 插件；设置新增「插件」页，右键笔记可切换正文。插件使用独立目录保存清单、入口、依赖、资源和运行状态；插件不提供安全沙箱，请只安装可信插件。

- 插件协议最终统一为 1.8，宿主只接受 1.8 协议插件。插件可按清单声明 Paper、Todo 与 Note 的读取、动态监听和受控写入能力，并可提供运行时标题、独立设置与状态迁移；插件缺失、加载失败或旧状态无法解析时会保留原有数据并使用可用回退。

- 插件可为胶囊提交最多三个任意顺序的标准组件，包括文字、图标、状态点、进度环和进度条；胶囊既可指定固定内容宽度，也可按实际组件自动适配。原生插件还可在固定高度的内容区使用纯 WPF 自绘动态图形，PaperTodo 继续统一负责外壳、关闭区、贴边和鼠标交互。

- 插件可为边缘快速浏览提供专属迷你界面：原生插件可创建轻量 WPF 迷你视图，Web 插件可声明独立本地迷你入口；未提供专属迷你界面的插件会使用自绘胶囊、标准组件或文字内容生成合适的回退。允许正文迁移的纯 WPF 插件可复用真实界面或最近一次内存快照，减少首次浏览时的重排等待。

- 插件清单可声明由设置控制的启动纸片；PaperTodo 完成常规恢复后统一创建或恢复对应插件纸，并通过「插件 ID + 实例键」避免重复实例。插件运行时标题会保留最后一次有效值，隐藏或尚未重新加载的插件纸片也能在关联待办和胶囊中显示；插件还可明确占用 Esc 与正文右键菜单，退出会话后自动归还。

- 随附原生时钟、专注计时器、复盘记录池、云原神与 Web 时钟示例，均适配窄窗口和胶囊显示；专注计时器可从边缘迷你界面直接开始或暂停，时钟与云原神提供各自的轻量状态界面；复盘记录池按创建、完成、重新打开、删除与恢复事件保存记录，旧状态会自动迁移并保留有效数据。

- Web 插件仅向自身本地顶层页面注入 PaperTodo 桥接；支持常规 WebView2 导航、远程框架、弹窗和权限请求，外部链接与常规网络下载交给默认浏览器，会话内生成的下载继续由 WebView2 处理；状态变化会即时保存，已加载插件文件发生变化或删除时会提示重新启动。

**设置与交互**

- 设置页新增界面语言下拉选择，可选跟随系统、简体中文、English、日本语或韩语；选择会保存，重启 PaperTodo 后生效。

- “快捷键置底”更名为“高级快捷键”；高级快捷键新增全部便签锁定、全部纸片透明、全部胶囊透明和当前焦点纸片透明。全部胶囊透明同时覆盖主胶囊和之后创建的胶囊，锁定覆盖插件便签并延续到之后创建的贴边胶囊，可选择是否允许点击锁头解锁；快捷透明度独立于空闲半透明。

- 全局快捷键可选择是否区分主键盘数字键与小键盘数字键；关闭区分时两者混合响应但不改写已保存绑定，切换模式遇到冲突时会保留现有快捷键。

- “失去焦点时自动收起纸片”新增严格收起：新建或显示后尚未使用的纸片在转去其他操作时会立即收起。实验室同时可让贴边胶囊与主胶囊不再保持置顶，展开纸片仍遵循自身置顶设置。

- 设置页的普通开关、实验室依赖项和高级快捷键录制会尽量只刷新当前区域，减少切换设置时的闪动和编辑中断；高级模式新增项使用更明显的分组视觉，视觉页可调整边缘胶囊间距。

- 边缘胶囊较窄、标准、较宽三档间距调整为 0、4、8 DIP，展开纸片、普通浮动胶囊、拖出的浮动边缘胶囊和主/贴边胶囊使用更清晰的阴影层次。

**笔记与 Markdown**

- Markdown 链接和裸 `http://` / `https://` 地址均可直接点击打开；行内代码和围栏代码中的示例地址保持普通文本，强调标记不会混入实际网址。

- Markdown 行内样式支持粗斜体 `***文本***` / `___文本___`，粗体、斜体、删除线和链接标签可以自然组合；反斜杠可转义 Markdown 标点。

**待办**

- 待办项可拖入一个文件或文件夹作为快启路径；路径只保存链接、不复制内容，并与纸片关联互斥。路径失效时显示警告色感叹号，可从右键菜单打开、打开所在位置或解除关联；快启名称会跟随关联纸片名称与长标题设置，并可在短标题模式只显示扩展名。

- 可从待办项左侧窄选区连续扫选多个完整项目，跨行拖选会继续按纵向位置扩展范围并自动滚动；多选文字区之外会保留每条已选待办的深色整行预选底色，文字选区只绘制在原有文字上且不改变文字颜色；多选后可一次复制、批量完成或恢复、右键删除全部，或拖动任一已选项的排序把手到底部垃圾区整组删除。

- 仅挂有快启路径、提醒或纸片关联的空文字待办也会继续保留，不会被退格或删除空纸片流程当作普通空项静默清除。

- 新增“已完成待办自动置底”：完成待办时移到已完成区域末尾，取消完成时移到未完成区域末尾；开启“完成后自动清除待办”时暂时停用自动置底，但会保留原设置。

- 待办关联从“关联笔记”扩展为“关联纸片”，可指向笔记、插件正文或其他待办纸；普通拖拽、MCP 与插件使用相同关联规则，删除目标纸片会统一解除关联。被待办关联的笔记纸或待办纸也可按设置退出胶囊队列。

**实验室功能**

- 新增本地 MCP 接口：由同一个 `PaperTodo.exe` 以 `--mcp` 模式提供，无需额外分发 MCP 程序。启用后默认只读，并可分别允许空白/追加写入、完整写入和直接删除；空白/追加写入可新建纸片、向已有待办纸新增待办及填充或追加笔记。直接删除默认关闭，开启即视为预授权；设置页可分别复制 JSON 连接配置与指导 AI 使用 PaperTodo 工具的 Skill 文本。

- 高级模式开启后，设置窗口新增第四栏「实验室」；实验功能按失焦/静置行为、窗口绑定/可见性联动、边缘磁吸等分组，每项独立开关，关闭后不参与普通纸片和胶囊运行。

- 可分别设置失焦纸片与静止胶囊的透明度，并可让展开纸片在失去焦点后自动收起；普通胶囊和贴边胶囊默认在悬停、激活或拖动时恢复实体，也可选择激活时继续透明或把相同透明度覆盖到主胶囊。还可在纸片失去焦点时仅隐藏顶栏操作按钮，或让普通浮动纸片从顶部收短整条标题栏并保持正文与底边位置不动。

- 可设置全局快捷键，将当前纸片或全部纸片、普通胶囊、贴边胶囊和主胶囊切换为鼠标点击穿透并置底；再次按键、关闭实验开关、隐藏纸片或使用托盘恢复入口都会退出该状态。

- 可启用轻量待办提醒：支持默认间隔、10/30/60 分钟、当天傍晚、次日早晨和自定义时间，待办行显示可点击倒计时并可随时修改或取消；到期时会打开并定位对应待办、通过系统托盘提示，并持续高亮或动效标记到用户处理该项。提醒可选择 Windows 系统提示音，程序目录中的有效 `papertodo.wav` 会优先作为自定义提醒声音；系统时间调整、休眠恢复和解锁后会立即重新检查，交付失败时会自动重试。可单独隐藏铃铛按钮，关闭提醒功能时停止计时但保留已保存时间。

- 可为普通胶囊启用屏幕或外部窗口边缘磁吸：拖入触发范围会显示吸附落点，松手后平滑随目标移动并自动收进目标边缘，只留可操作的小头；鼠标移入滑出，移开后短暂延迟再收回，重新移入可取消收回。目标全屏或外侧空间不足时，小头保持在屏幕内侧；同时开启窗口绑定后，打开磁吸胶囊会无缝转为对同一窗口的绑定并继续跟随。重新拖动、目标失效或关闭功能时自动解除，不影响贴边队列和主胶囊。

- 可把待办纸或笔记纸顶栏的「窗口绑定」按钮拖到其他软件窗口，松手后纸片按自动或指定边缘贴合并平滑跟随；可拖动改绑、右键解除，并沿目标边缘调整相对位置，同时保留更大的向外拖离距离，减少调整位置时误解除。目标靠近屏幕边缘时允许纸片部分出屏，但会保留可操作的顶栏区域；绑定期间原贴边胶囊槽位暂时让出，解除后自动恢复。窗口绑定可联动目标的最小化与激活：目标不可见时临时隐藏纸片或显示独立胶囊，目标恢复后无激活地重新显示纸片；绑定、恢复和解除过程保留纸片原有的展开、胶囊与可见性状态。

- 可在 Windows 10 及以上系统启用当前虚拟桌面唤醒：主动显示纸片、从托盘唤醒或打开普通/贴边胶囊时，可将 PaperTodo 自有窗口移到当前桌面；不会切换系统桌面或移动外部软件。该实验与“从窗口切换中隐藏纸片”互斥。

**优化**

- 优化高刷新率和高 DPI 下的贴边胶囊动画、显示器切换和窗口跟随体验，快速连续操作时的队列移动与胶囊形变更稳定。

- **优化窄纸片顶栏按钮与标题防重叠响应式布局**：根据顶栏实际可用宽度分阶段收起右侧功能按钮，并始终保留关闭/折叠按钮；空间极窄时再按需收起标题，避免窄尺寸下元素重叠或反复闪烁，同时支持更窄的纸片最小宽度。

- **优化顶栏图标与纸片右键菜单结构**：顶栏、关联状态和托盘常用图标在不同 DPI 下显示更清晰；纸片右键菜单增加“快捷”分组与新建、设置入口，任何状态下均保留“设置”，胶囊恢复统一显示为“展开为纸片”，“删除纸片”使用更明确的危险提示，笔记纸右键菜单增加外部 MD 编辑器入口。

- 纸片主体、胶囊、贴边宿主和 Markdown 预览区的右键菜单会随折叠、模式和窗口状态同步更新，状态相关菜单项保持一致。

- 全屏避让对前台窗口变化响应更及时，关闭避让后停止额外检查，减少不必要的后台开销。

**bug修复和边界修正**

- 修复通过全局快捷键、托盘“显示全部”或显隐切换重新显示纸片时，开启“记住边缘胶囊展开位置”的纸片可能无法回到上次展开位置的问题；同时优化多纸片批量恢复，减少重复胶囊重排，并同步刷新待办关联纸片的显示状态。

- 旧版 Windows 兼容性：修复部分 Windows 10 旧版本环境下 PaperTodo 启动时可能直接崩溃、无法进入程序的问题。

- 修复 Unity 等无边框全屏程序前台运行时，主胶囊、边缘胶囊及悬浮提示仍可能压在画面上方的问题；退出全屏后照常恢复置顶。

- 修复把边缘胶囊拖到另一台显示器后，点击展开时纸片仍可能回到原显示器的问题。

- 修复关闭贴边胶囊模式时会同时清除“显示主胶囊”偏好的问题；重新启用贴边模式后会继续沿用原设置。

- 修复程序重启后，直接从未展开的贴边胶囊右键删除非空纸片时，删除确认框可能导致异常退出的问题。

- 修复任务栏托盘菜单在程序首次打开或 Windows 显示缩放切换后的第一次右键中，可能被夹到屏幕右下角或闪现后将焦点退回纸片的问题；托盘菜单会按实时鼠标位置、当前显示器缩放与工作区正确放置。

- 修复设置页选择菜单未完整跟随当前主题的问题。
