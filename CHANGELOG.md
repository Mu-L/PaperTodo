...
- 多显示器、左右双边队列同时存在时，序号应指向哪条队列仍需补充完整规则；当前快捷键面板保留 1-9 的配置、默认键位和冲突检测，但在规则确定前不会执行边缘胶囊打开行为。

补齐readme

1. 版本 badge：v2.1 → v2.3
1. 置顶按钮：类型图标 → 图钉图标
1. 新增 Windows 分屏贴靠说明
...
- 开启胶囊模式后，展开纸片可在标题、待办或笔记编辑中直接按一次 Escape 提交当前内容并收回胶囊，不再先退出编辑态。
- 设置窗口内新增可互相跳转的快捷键页面，托盘菜单也可直接打开；支持为显示全部、隐藏全部、切换显隐、新建待办、新建笔记和退出六种全局操作录入、清空或恢复快捷键。
- 保存快捷键前会同时检查应用内重复和系统占用；任一组合不可用时整批保持原配置和原快捷键，不会只应用其中一部分。边缘序列 1-9 已预留默认 Ctrl+Alt+1 至 Ctrl+Alt+9，但在队列选择规则确定前保持停用。
- 可设置边缘胶囊标题最多测量前 N 个 Unicode 文本字符，标题内容和普通胶囊保持不变；默认“全部”兼容原有宽度。

**笔记图片**

- 笔记支持从剪贴板粘贴图片、拖入图片文件或从菜单插入图片，编辑态和浏览态都会按纸片宽度完整显示图片；图片右键菜单会跟随纸片主题，点击渲染图片可选中整个图片对象，并通过 Backspace 或 Delete 直接删除对应的 Markdown 引用。
- 图片会显示在对应 Markdown 图片语法下方，新插入图片默认使用 `![image|100%](i:001)` 这类短编号语法，并支持通过 `![image|宽x高, 75%](...)` 或 `{width=75%}` 调整显示大小；复制、粘贴和撤销不会夹带隐藏字符，把图片引用粘贴到另一笔记时会复制对应图片数据，不再依赖原笔记。
- 图片随本地数据保存，删除图片引用后仍可在本次编辑中撤销，退出时会清理不再引用的图片数据；图片数据异常时会优先从有效备份恢复，外部打开笔记时会安全导出临时图片文件，方便普通 Markdown 编辑器显示。
- 批量插入图片会在整批处理成功后一次写入；任一图片失败时整批回滚，不会留下未插入正文的半批图片数据。
- 过大的源图片文件会在读取前被拒绝；外部打开笔记时如果临时图片无法完整导出，会明确提示并取消打开，避免外部编辑器收到无法显示的内部图片地址。

**胶囊相关改动**

- 拖动边缘胶囊调整顺序或跨队列移动时，当前胶囊会保持在其他胶囊上方，不再被经过的胶囊遮挡。
- 关闭胶囊模式时，已折叠纸片会正常展开回纸片形态，不再跳过展开过渡。

**bug修复和边界修正**

- 优化应用冷启动和多纸片恢复速度，减少启动阶段的重复窗口准备工作，并让精简版使用预编译代码更快显示纸片。
- 修复应用启动恢复耗时较长时，其他启动参数可能静默丢失的问题；新建纸片、显示和退出等命令会等待启动完成后按顺序执行。
- 通过启动参数、托盘或首次启动新建纸片时，新纸片会创建在鼠标所在显示器，并加入该显示器对应的默认边缘队列。
- 修复连续新建多张纸片时，超过一定数量后新纸片会叠在同一位置的问题，现在会自动避开已有纸片错开摆放。
- 修复笔记浏览态下按 Escape 无法收回胶囊的问题，现在编辑态和浏览态按 Escape 都能收回。
- 剪贴板中只有图片（如截图）时可直接按 Ctrl+V 粘贴图片；同时包含文本和图片时会优先粘贴文本，避免富文本内容被误插入为截图。
- 修复编辑态下对图片点右键会被误判为失焦、自动切回浏览态的问题，图片菜单关闭后会停留在编辑态。
- 最小化的纸片可通过托盘单项、“显示全部”或关联笔记入口正常恢复，不再被误判为已经显示。
- 边缘胶囊的右键菜单会稳定显示在其他置顶窗口之上，不再偶发被窗口遮挡。
- 边缘胶囊打开右键菜单后会保持外移，不再因鼠标进入菜单而缩回；拖动排序期间隐藏纸片或关闭胶囊模式，也不会残留排序状态影响下次显示。
- 修复关联笔记改绑、删除空待办以及 Undo/Redo 后胶囊显隐可能没有及时更新的问题；多行粘贴待办现在只需撤销一次即可完整恢复。
- Markdown 代码块内的内部图片语法不再被当作真实图片处理；清理图片数据时也会核对图片所属笔记，避免错误引用影响资产保留。
- 优化 Windows 分屏贴靠显示：贴靠时自动隐藏纸片阴影和边距，并补充识别三列中间格、2/3 主列与右侧上下小格等 Windows 11 常见布局。
- 分屏纸片折叠为胶囊后即视为普通纸片，再次展开会恢复到分屏前记录的纸片大小；之后隐藏再显示也不会突然跳回旧分屏格子。
- 退出时会可靠回收脚本胶囊的常驻 PowerShell 进程；单个窗口或清理步骤异常也不会再阻断 PaperTodo 完整退出。
- 修复纸片缩放到最小尺寸附近时可能越过尺寸限制并错误裁切内容的问题。
