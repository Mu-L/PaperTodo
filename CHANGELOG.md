...
- **正文插件生态**：笔记纸支持切换为本地 Web 插件或原生 WPF 插件，设置中新增独立「插件」管理页；宿主升级为 2.0 协议并向下兼容 1.8 插件；插件不提供安全沙箱，仅建议安装可信来源的插件。
  - **协议 2.0 全面能力**：插件可按需声明 Paper/Todo/Note 数据读写权限，支持自定义顶栏操作按钮、注册全局快捷键、控制所属纸片显隐/折叠展开/激活、共享 App 后台运行时以及独立配置面板，并支持按设置自动创建或恢复专属启动纸片。
...
- **失焦严格收起**：新增“失去焦点时严格自动收起”选项，新建或唤出后尚未进行操作的纸片，在切换到其他应用时会立即自动收回。
...
- **虚拟桌面唤醒**：在 Windows 10/11 多桌面环境下唤醒纸片时，可将纸片自动移至当前活跃桌面，无需手动切回原桌面。
...
- **小键盘数字键选项**：全局快捷键支持设置是否区分主键盘数字键与小键盘数字键（默认保持独立响应）。
