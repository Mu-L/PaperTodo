...
- 内置待办、Markdown 与插件默认卡片统一通过同一个内部内容入口提供尺寸和实时内容；插件协议仍保持 1.7，未预设预览形态的插件先使用标题和胶囊状态回退，不使用截图。

**插件协议 1.7**

- 插件协议升级到 1.7，宿主只接受 1.7 协议插件。插件仍依附于一张笔记纸，但所属纸片、展开正文和整个工作区的能力范围更加明确，正式标题、展开态运行时标题与胶囊表现不再被强制绑在一起。
...
- 原生插件可在固定高度的胶囊内容区自由绘制动态图形，普通胶囊与贴边胶囊分别创建独立视图；外壳和全部鼠标交互仍由 PaperTodo 掌控，自绘失败自动退回标准组件模板。
