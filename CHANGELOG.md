...
- 边缘胶囊新增可交互的预览卡片形态：鼠标移入后立即按内容请求的合理宽高展开，并真实占用队列空间；完整纸片已经展开且保留边缘胶囊时，也可以从边缘快速打开预览。

- 连续浏览时，当前卡片在鼠标离开后继续保持；进入另一枚胶囊并短暂停稳后才原子切换预览对象，首次从屏幕边缘进入仍立即展开，离开整条边缘浏览区域则不等待停稳直接收回。向队列上方浏览时，后续胶囊会按新卡片高度补位；向下浏览仍保留目标下方次序，避免跨项切换时发生视觉错序。
