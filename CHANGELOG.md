...
- 设置窗口新增「通用 / 快捷键」页签，支持为显示全部、隐藏全部、切换显隐、新建待办、新建笔记、退出录入全局快捷键。
- 保存快捷键前会同时检查应用内重复和系统占用，任一组合不可用时保持原有配置。
- 可限制边缘胶囊标题最多显示前 N 个 Unicode 字符，默认“全部”兼容原有宽度。

**笔记图片**

- 笔记支持从剪贴板、拖入或菜单插入图片，按纸片宽度完整显示；选中图片后可通过删除对应 Markdown 引用。
- 新插入图片默认使用 `![image|100%](i:001)` 短编号语法，支持 `![image|宽x高, 75%](...)` 或 `{width=75%}` 调整显示大小；复制、粘贴和撤销不会夹带隐藏字符，把图片引用粘贴到另一笔记时会复制对应图片数据，不再依赖原笔记。
- 图片以增量方式存储在单个本地资产文件中，不会因图片增多而反复重写全部数据；单张图片元数据或内容损坏时只隔离该图片并在原位置显示损坏状态，不影响其他图片继续读取和写入；正文保存成功后清理当前笔记数据已不再引用的图片，外部打开笔记时安全导出临时文件。
...
- 边缘胶囊与 slot 0 主胶囊改为按实际显示宽度绘制单侧圆角贴边标签，并在目标显示器坐标中一次提交完整物理像素矩形；关闭区固定在屏幕墙边并随窗口实际宽度同步伸缩，拖出队列时改用不含关闭区的完整悬浮胶囊，贴边胶囊不再通过裁切、屏幕外溢出或复用悬浮布局形成外观，修复竖屏、混合缩放、右侧悬停闪烁及拖拽归位后的裁切和形态残留。

**bug修复和边界修正**

- 优化冷启动和多纸片恢复速度，精简版使用预编译代码更快显示纸片。
- 修复应用启动恢复耗时较长时，其他启动参数可能静默丢失的问题
- 通过启动参数、托盘或首次启动新建的纸片，会创建在鼠标所在显示器并加入对应边缘队列。
- 连续新建多张纸片时自动错开位置，不再重叠在同一位置。
- 修复最小化的纸片无法恢复显示的问题。
- 边缘胶囊右键菜单稳定显示在置顶窗口之上，不再偶发被遮挡。
- 边缘胶囊打开右键菜单后保持外移，不会因鼠标进入菜单而缩回；拖动排序期间隐藏纸片或关闭胶囊模式不残留排序状态。
- 拖动排序期间发生的队列重排和标题宽度刷新会在拖动结束后统一应用；伸缩中的标题变化会平滑接续，不再丢失刷新或硬切关闭区。
- 边缘胶囊关闭区恢复到屏幕墙边；左右胶囊都按真实可交互区域判断悬停，移开后会稳定收回，右侧伸缩不再因原生窗口移动产生反复悬停切换和位置闪烁。
...
- 笔记达到 100000 字符后不再后台截掉已有正文；继续输入会被阻止，超过单次 30000 字符、单行 6000 字符或笔记总上限的粘贴会整次拒绝并给出一次提示。
