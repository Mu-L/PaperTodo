...
- 笔记正文支持内置 Markdown、可信原生 WPF 与本地 Web 双插件；设置新增「插件」页，右键笔记可切换正文。每个插件使用独立的 `plugins/<插件 ID>/` 目录，清单、入口、依赖、资源和该插件的 `.runtime` 数据放在一起。

- Web 插件限制在自身虚拟源中运行，外链交给默认浏览器，并拦截远程框架、弹窗、下载与权限请求；状态变化必须即时保存，版本迁移、进程失败和缺失插件都有明确回退。原生插件每个正文会话使用新的工厂实例，已加载 DLL 的变化或删除会提示重启。

- 内置 Markdown 的编辑器、焦点、预览、图片与重建状态迁入独立正文会话，PaperWindow 只保留成熟交互方法的过渡转发，正文根节点在渲染模式重建时保持稳定。

**实验室功能**

- 新增本地 MCP 接口：由同一个 `PaperTodo.exe` 以 `--mcp` 模式提供，无需额外分发 MCP 程序。启用后默认只读，并可分别允许空白/追加写入、完整写入和删除；空白/追加写入可新建纸片、向已有待办纸新增待办及填充或追加笔记，删除仍需在 PaperTodo 中确认。
...
- 可在 Windows 10 及以上系统启用当前虚拟桌面唤醒：主动显示纸片、从托盘唤醒或打开普通/贴边胶囊时，可将 PaperTodo 自有窗口移到当前桌面；不会切换系统桌面或移动外部软件，关闭开关后立即释放接口。
...
- 优化待办提醒可靠性与触发重试：打开目标纸片或弹出托盘通知成功前保留提醒数据，交付均失败时自动重试，避免系统通知区域未就绪静默丢提醒；加强 MCP 本地管道读写与客户端响应超时拦截，防止异常连接卡死。

- 优化胶囊边缘吸附与窗口系留防抖：鼠标离开磁吸胶囊后增加 180ms 延迟收回机制并可随时再次取消，防止边缘悬停抖动；优化外部窗口枚举节流，并将沿边拖离解绑阈值调大至 72 DIP，提升拖拽操作稳定性。
