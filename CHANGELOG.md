...
- 新增本地 MCP 接口：由同一个 `PaperTodo.exe` 以 `--mcp` 模式提供，无需额外分发 MCP 程序。启用后默认只读，并可分别允许空白/追加写入、完整写入和直接删除；空白/追加写入可新建纸片、向已有待办纸新增待办及填充或追加笔记。直接删除默认关闭，开启即视为预授权，不再逐次弹窗确认。

- 高级模式开启后，设置窗口新增第四栏「实验室」；内容改为整页双列，并把失焦/静置行为与窗口绑定/可见性联动/边缘磁吸分别收束为两组。每项实验功能独立开关，关闭后不参与普通纸片和胶囊的运行；实验室仍在原有设置窗口高度内滚动，不会改变窗口尺寸。

- 可分别设置失焦纸片与静止胶囊的透明度，并可让展开纸片在失去焦点后自动收起；拖拽、标题编辑、菜单或下拉栏交互不会误触发自动收起。普通胶囊和贴边胶囊会在悬停、激活或拖动时恢复实体，主胶囊不受影响。
...
- 优化待办提醒可靠性与触发重试：打开目标纸片或弹出托盘通知成功前保留提醒数据，交付均失败时自动重试，避免系统通知区域未就绪静默丢提醒；完成待办时始终清除旧提醒；MCP 客户端无响应等待上限缩短为 10 秒。
