...
- **笔记图片缓存**：同一张笔记图片只保留一份解码缓存，并按最近使用淘汰：最多约 20 张或约 50 MB 解码像素（先到先限），避免长笔记滚动时解码内存无限堆积；拖动调整窗口大小时用低质量临时缩放跟手（不整页重解码），停下后再按当前宽度调整清晰度（可变大也可变小）；笔记隐藏、折叠、最小化或关闭后释放该笔记的图片缓存。从胶囊展开时在正文开始显示前就恢复图片渲染，避免动画期间短暂露出原始图片引用。
