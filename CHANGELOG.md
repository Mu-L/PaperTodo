...
- 可在实验室启用轻量待办提醒：支持从待办行按钮或右键菜单选择默认间隔、10/30/60 分钟、当天傍晚、次日早晨和自定义时间；设置后在待办行显示简洁倒计时，到期时通过系统托盘单次提示。可单独隐藏铃铛按钮、仅保留右键入口；关闭提醒功能时停止计时并隐藏入口，已保存的时间保持不变。

- 可为普通胶囊启用屏幕或外部窗口边缘磁吸；吸附外部窗口后会随其移动，重新拖动、目标失效或关闭功能时自动解除，不影响贴边队列和主胶囊。

- 可从展开纸片的右键菜单选择一个外部窗口进行系留，纸片会按自动或指定边缘贴合并随目标移动；展开纸片原有的贴边胶囊槽位会在系留期间暂时让出，并在解除后自动恢复。重新拖动、目标失效、手动解除或关闭实验开关都会停止跟随。

- 系留可选联动目标的最小化与激活：目标不可见时临时隐藏纸片或显示独立胶囊，目标恢复后无激活地重新显示纸片；该过程不改写纸片原有的展开、胶囊或可见性数据。
