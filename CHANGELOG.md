...
- 设置窗口拆分为「基础行为 / 视觉 / 快捷键」三页，窗口高度固定，切换分页时不再忽高忽低；Markdown 显示仍在基础行为页。视觉页可按 5% 调整 80%–120% 的整体字体大小，并分别设置笔记正文、待办正文、标题和胶囊文字的小中大字号补正与加粗。单个笔记原有的百分比缩放会在全局与正文补正基础上继续叠加，待办行的内边距、最小高度、勾选列和勾选框也会随整体字体大小同步缩放。

**胶囊快捷操作与快捷键**

- 胶囊模式下，无论纸片处于编辑态还是浏览态，按一次 Escape 即可提交当前编辑内容并收回胶囊。
- 快捷键页支持为显示全部、隐藏全部、切换显隐、新建待办、新建笔记、退出录入全局快捷键；录入已经注册的组合键时会将其作为新输入，不会误执行原命令。
- 每项全局快捷键均可独立启用；录入、清空、恢复默认、开关切换都会立即生效并自动保存，不再需要单独点保存。录入组合键时会自动勾选启用；单项或整表恢复默认时按默认启用状态写入（边缘序列默认关闭）。新增默认关闭的左侧优先 `Ctrl+Shift+1–9` 和右侧优先 `Ctrl+Alt+1–9` 边缘胶囊序列：每侧只占一行设置，两枚修饰键可自定义、数字 1–9 固定，开关一次启用整组。调用时优先查找鼠标所在显示器的指定侧与对侧，再按左邻、右邻顺序搜索其他显示器，主胶囊不计入序号；拖动任意边缘胶囊期间会忽略序列快捷键。
- 应用快捷键前会同时检查应用内重复和系统占用，任一组合不可用时保持原有配置。
