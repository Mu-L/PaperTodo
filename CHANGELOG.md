...
- 启动恢复多张贴边胶囊时，先按完整队列统一确定槽位并提交首帧，避免重复安排和前台纸片的胶囊抢先出现；启动或稍后显示折叠纸片时，胶囊显示后再逐张预创建仍处于显示状态的纸片内容，减少纸片较多时等待首个可用界面的时间，同时保持后续展开速度。

- **笔记图片缓存**：同一张笔记图片只保留一份解码缓存，并按最近使用淘汰：最多约 20 张或约 50 MB 解码像素（先到先限），**当前视口内正在显示的图片不受该上限限制**；拖动调整窗口大小时复用当前清晰位图即时缩放（不整页重解码，并保持各图原有的固定、百分比或自然宽度规则），不再为了跟手刻意降低缩放质量；停下后按当前显示器 DPI 和实际显示宽度精确解码，不使用宽度分桶或近似尺寸，跨不同缩放比例的显示器后也会按新 DPI 重建；笔记隐藏、折叠、最小化或关闭后释放该笔记的图片缓存。从胶囊展开时在正文开始显示前就恢复图片渲染，避免动画期间短暂露出原始图片引用；最小化后再隐藏的笔记重新显示时也会正确恢复图片。

**bug修复和边界修正**

- 修复从托盘或主胶囊菜单退出时，菜单画面可能在同步保存期间短暂残留的问题，同时保持普通托盘操作的及时响应。

- 修复启动时忽略已隐藏纸片/胶囊的 `isVisible` 配置，重新打开后仍会显示的问题；隐藏状态会按数据文件正确恢复，托盘「显示全部」仍可一次找回。
