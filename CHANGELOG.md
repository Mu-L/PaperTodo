...
- 可在实验室启用轻量待办提醒：支持从待办行按钮或右键菜单选择默认间隔、10/30/60 分钟、当天傍晚、次日早晨和自定义时间；自定义时间使用紧凑的纸片风格输入框，可直接输入日期和时间，并按天或按分钟微调。设置后在待办行显示可点击的简洁倒计时，可随时修改或取消；到期时会打开并定位对应待办，同时通过系统托盘单次提示。系统时间调整、休眠恢复和解锁后会立即重新检查，平时仍以最长一分钟的轻量轮询兜底。可单独隐藏铃铛按钮、仅保留右键入口；关闭提醒功能时停止计时并隐藏入口，已保存的时间保持不变。

- 可为普通胶囊启用屏幕或外部窗口边缘磁吸；拖动进入触发范围后会显示吸附落点，松手完成吸附，完全被其他窗口遮住的目标不会参与吸附。吸附外部窗口后会平滑随其移动并自动收进目标边缘，只留一个可操作的小头；鼠标移入时滑出，移开后自动收回，不保留额外激发状态。目标全屏或外侧空间不足时，小头会留在屏幕内侧。重新拖动、目标失效或关闭功能时自动解除，不影响贴边队列和主胶囊。

- 可把待办纸或笔记纸顶栏的「窗口绑定」按钮拖到其他软件窗口，松手后纸片会按自动或指定边缘贴合并平滑随目标移动；按钮会明确显示未绑定与已绑定状态，拖动按钮可改绑，右键按钮可解除。绑定后沿目标边缘轻拖标题栏可调整位置，只有明显向外拉开才会解除绑定。目标靠近屏幕边缘时纸片即使部分超出屏幕也会继续跟随，并保留可操作的顶栏区域。展开纸片原有的贴边胶囊槽位会在绑定期间暂时让出，并在解除后自动恢复。
