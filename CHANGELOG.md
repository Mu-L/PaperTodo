...
- 检查代码后，实际情况是：
  ...
  PaperTodo 的雅黑、等线预设都把 Segoe UI 排在第一位。
  WPF 会使用回退链中第一个包含该字符的字体；数字 1 因此实际由 Segoe UI 绘制，而不是雅黑或等线。WPF 字体回退规则
  开启加粗时，代码只把 Normal 换成 SemiBold，没有主动调整字距或横向缩放。
  Segoe UI Semibold 的数字确实存在整体间距明显收紧的问题，微软字体文档仓库中也有人报告了完全相同的现象。Segoe UI 数字间距问题

  所以准确结论是：这是 Segoe UI Regular 与 Semibold 字形度量不同造成的真实宽度缩小，不是视觉错觉，也不是雅黑或等线本身导致的。
...
-  窗口系留模式
  目前纸片只能常驻于桌面或置顶于所有窗口之上。工作流中，便签是服务于某个特定具体任务的（比如对着一段产品文档写代码，或者看着参考图作画）。
  
  **全新交互**：新增“系留”功能。可以把一张纸片像吸铁石一样，贴到任意一个窗口的边缘
  
  **工作流联动**：纸片将动态跟随目标软件。目标软件最小化，纸片自动化为胶囊或隐藏；目标软件切到前台，纸片同步在它的边缘滑出。
  
  **技术实现**：利用 Win32 的 `SetWinEventHook` 监听目标窗口的句柄（HWND）状态（移动、最小化、激活），在 WPF 层实现无感知的坐标联动，真正做到“如影随形”却不干扰全局桌面。
  
- **幽灵壁纸模式**：不占层级的无感存在，  

  **全新交互**：开启后，纸片会“半透明融化”进你的桌面壁纸里。它不占用任何合法的窗口层级，平时完全鼠标穿透，你拖动桌面图标或框选文件时，它就像纹身一样印在壁纸上。

  唤醒机制：当按住一个自定义全局修饰键（比如长按 Caps Lock 或连续晃动鼠标），幽灵纸片恢复全实体并获取输入焦点，失去焦点即再次隐形。

  **技术实现**：通过修改窗口扩展样式 WS_EX_TRANSPARENT（鼠标穿透）和调整 DWM 的恶魔级 Z-order（将窗口层级强行插在系统 Progman / WorkerW 壁纸层之上），实现极致纯净的原生桌面看板效果。

- **普通胶囊磁吸吸附**：评估为非侧边队列的普通胶囊增加轻量磁吸能力

  拖动靠近窗口或屏幕边缘时可吸附，并在目标移动时尝试跟随；不影响侧边胶囊队列、主胶囊和纸片展开逻辑，拖离或目标失效后自动解除吸附。
...
- 程序启动时会自动回收已清理图片留下的空号，后续插入的图片优先使用这些编号；已有引用与备份中仍出现的编号不会被复用，已有图片编号保持不变。
- 笔记图片支持 WebP（拖入、粘贴与选图）；系统无法解码时沿用现有导入失败提示。

**bug修复和边界修正**

- 修复边缘胶囊拖离队列后的原生拖拽判定：在管理员权限应用位于前台时仍可正常跨边缘、跨屏和排序。
- 修复混合 DPI 多显示器下边缘胶囊原生拖拽起拖锚点偏移：浮动胶囊在 System Aware 坐标中对齐光标，落点仍按应用的 PerMonitorV2 设备坐标采样，避免跨缩放屏时抓住瞬间错位。
- 修复修改纸片标题后，展开纸片会被错误移动到贴边胶囊附近并保存该位置的问题。
- 修复从资源管理器拖入或通过右键菜单选择图片时，笔记因临时进入浏览态而取消插入的问题。
- 修复在资源管理器复制本地图片文件后，无法直接粘贴到笔记中的问题。
- 修复截图等剪贴板位图粘贴到笔记后整图透明的问题（优先使用 PNG 等编码格式，并校正 alpha 全为 0 的位图）。
- 拖入或粘贴非图片文件时不再误走笔记插图流程。
