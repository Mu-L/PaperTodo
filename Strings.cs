...
public static class Strings
{
    ...
    private static readonly IReadOnlyDictionary<string, string[]> Supplemental =
        new Dictionary<string, string[]>(StringComparer.Ordinal)
        {
            ...
            ["TipLabsHideInactiveTitleBar"] = ["纸片失去焦点时隐藏整条标题栏；悬停或重新激活时显示，并保留标题栏占用空间。", "Hide the whole title bar while the paper is inactive; reveal it on hover or activation while preserving its layout space.", "紙が非アクティブな間はタイトルバー全体を隠し、ホバーまたは再アクティブ化で表示します。タイトルバーの領域は保持します。", "메모가 비활성일 때 제목 표시줄 전체를 숨기고, 마우스를 올리거나 다시 활성화하면 표시합니다. 제목 표시줄 공간은 유지합니다."],
