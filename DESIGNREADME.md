...
- Release 资产直接上传 exe，并附带 `SHA256SUMS.txt`，不再套 zip。
- 两个 exe 都使用 GitHub Artifact Attestations 发布原产地签名；这是 provenance 签名，不是 Windows Authenticode 代码签名。
...
```powershell
dotnet publish .\PaperTodo.csproj -c Release -r win-x64 --self-contained true -o 输出\PaperTodo-v1.3-win-x64-self-contained-compressed\ -p:PublishSingleFile=true -p:PublishReadyToRun=true -p:IncludeNativeLibrariesForSelfExtract=true -p:EnableCompressionInSingleFile=true -p:PublishTrimmed=false -p:DebugType=none -p:DebugSymbols=false
```
...
```powershell
dotnet publish .\PaperTodo.csproj -c Release -r win-x64 --self-contained false -o 输出\PaperTodo-v1.3-win-x64-framework-dependent\ -p:PublishSingleFile=true -p:PublishReadyToRun=false -p:PublishTrimmed=false -p:DebugType=none -p:DebugSymbols=false
```
...
- 发布后自动校验两个正式 exe 的 SHA256 和 Artifact Attestation。
