...
把 Markdig 解析结果转换成 WPF `FlowDocument`。

它不是完整 Markdown 编辑器，只支持当前产品需要的轻量预览。不要轻易引入图片、表格、附件、HTML 等复杂能力；这些会扩大安全、布局和体验复杂度。

### `MarkdownTextBox.cs`

笔记编辑用 TextBox 扩展，主要处理 Markdown 快捷键。
...
- `Topmost` 来自 `PaperData.AlwaysOnTop`
...
笔记纸使用 `MarkdownTextBox` 编辑，使用 `FlowDocumentScrollViewer` 预览。

基本模型：

- 编辑框和预览控件叠在同一个 Grid 里。
- 编辑时显示 TextBox。
- 预览时隐藏 TextBox，显示 FlowDocument。
- 内容保存在 `PaperData.Content`。
- Markdown 渲染结果会按文本和主题缓存。
...
WPF 动态资源能处理部分刷新，但不是全部。尤其是代码里动态生成的控件、Markdown `FlowDocument`、托盘菜单项，都需要主动重建或重新渲染。
...
```powershell
dotnet publish -c Release -r win-x64 --self-contained true -p:PublishSingleFile=true -p:PublishReadyToRun=true -p:IncludeNativeLibrariesForSelfExtract=true -p:EnableCompressionInSingleFile=true -p:PublishTrimmed=false -p:DebugType=none -p:DebugSymbols=false -p:OutputPath=输出\PaperTodo-v1.3-win-x64-compressed\
```
...
框架依赖、单文件、不 R2R、不 Trim。

参考参数：

```powershell
dotnet publish -c Release -r win-x64 --self-contained false -p:PublishSingleFile=true -p:PublishReadyToRun=false -p:PublishTrimmed=false -p:DebugType=none -p:DebugSymbols=false -p:OutputPath=输出\PaperTodo-v1.3-win-x64-fd-single-no-r2r\
```
...
- 更可靠的发布脚本，把两个正式包和 zip 一键产出。
- 发布前自动检查包内是否混入 `data.json`。
- 更轻的首次启动性能记录脚本。
