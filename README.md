...
- **原生纸片体验**：Markdown 预览、托盘菜单、拖拽排序和主题切换都使用 WPF 原生控件实现。
...
- 两种纸片：
  ...
  - **笔记纸**：普通文本 + Markdig Markdown 解析 + WPF 原生预览。
- 纸片支持移动、缩放、置顶、隐藏、删除。
- 胶囊模式：可将纸片折叠成小胶囊，减少桌面占用，并可从托盘统一启用 / 关闭。
...
- **预览状态右键**：包含新建纸片、折叠 / 恢复胶囊、隐藏、删除等纸片操作；点击预览内容可进入编辑。
...
- 接入 Markdig。
- 使用 Markdig AST 渲染 WPF 原生 `FlowDocument`。
- 不转 HTML，不使用 WebView。
- 笔记纸支持编辑 / 预览切换。
...
- 在笔记纸中增加 `FlowDocument` 预览缓存。
- 文本内容未变动时复用已渲染预览，降低频繁点击带来的 CPU 消耗。
- 清理 `PaperWindow.cs` 和 `MarkdownRenderer.cs` 中残留的冗余样式构建器。
...
- **性能与体验极致收拢**：内置 FlowDocument 预览缓存、分段式扁平托盘菜单，以纯 WPF 原生技术栈实现毫秒级动态换肤和流畅交互。
...
- **深浅色缓存穿透修复**：修复了“在编辑态切换主题导致旧版预览缓存不失效”的底层漏洞；现在的缓存校验不仅对比文本哈希，同时验证 `Theme.IsDark` 状态，确保笔记返回预览态时永远使用正确的主题配色渲染。
