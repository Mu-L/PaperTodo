...
internal sealed class PaperCommandService
{
        ...
        bool includeBlank = false)
    {
        ...
        return _controller.State.Papers
            ...
            .SelectMany(paper => paper.Items
                ...
                .Where(item => includeBlank || IsObservableTodo(item))
        ...
        PaperOperationContext context)
    {
        ...
        if (request.Text == null &&
            ...
            !request.UpdateLinkedNote)
        {
            throw Error(
                "invalid_params",
                "Provide text, done, order and/or updateLinkedNote.");
        }
        ...
        var text = request.Text == null
            ...
            : RequiredText(
                ...
                allowEmpty: true,
                "text");
        var linkedNoteId = NormalizeLinkedNoteUpdate(request);
        var snapshot = TodoPaperSnapshot.Capture(paper);
        ...
        IReadOnlyList<TodoCreateItem> inputs)
    {
        ...
        {
            var linkedNoteId = NormalizeLinkedNote(input.LinkedNoteId);
        ...
        bool allowEmpty)
    {
        ...
        foreach (var input in inputs)
        {
            ...
            _ = NormalizeLinkedNote(input.LinkedNoteId);
        }
    }

    private string? NormalizeLinkedNoteUpdate(UpdateTodoRequest request) =>
        request.UpdateLinkedNote
            ? NormalizeLinkedNote(request.LinkedNoteId)
            : null;

    private string? NormalizeLinkedNote(string? value)
    {
        ...
        if (!_controller.State.EnableTodoNoteLinks)
        {
            throw Error(
                "note_links_disabled",
                "Todo-note links are disabled in PaperTodo Settings.");
        }
        var id = RequiredId(value, "linkedNoteId");
        if (_controller.State.Papers.All(paper =>
                paper.Type != PaperTypes.Note ||
                !string.Equals(paper.Id, id, StringComparison.Ordinal)))
        {
            throw Error("note_not_found", "The linked note does not exist.");
        }
        return id;
    }
    ...
    private static bool IsBlankPlaceholderPaper(PaperData paper) =>
        paper.Items.Count == 1 && !IsObservableTodo(paper.Items[0]);

    private static bool IsObservableTodo(PaperItem item) =>
